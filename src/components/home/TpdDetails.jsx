...
import { packageDetails } from "../../data/TpDetails";
...
export default function PackageDetailsPage() {
  ...
  // Debugging: Log the packageId
  console.log("Package ID from URL:", packageId);
  console.log("Available package keys:", Object.keys(packageDetails));

  // Find the package by matching the numeric ID at the end of the key
  const pkgEntry = Object.entries(packageDetails).find(([key, value]) => {
    // Extract the numeric part from the key (e.g., "1" from "darjeeling-1")
    const keyId = key.split("-").pop();
    return keyId === packageId;
  });

  // Get the package object
  const pkg = pkgEntry ? pkgEntry[1] : null;

  // Debugging: Log the found package
  ...
  const handleInputChange = (e) => {
    ...
    setFormData({
      ...formData,
      [name]:
        name === "persons" || name === "children" ? parseInt(value) : value,
    });
  };
  ...
  const handleSubmit = (e) => {
    ...
    const whatsappNumber = "919337124745"; // must include country code (91 for India)
...
---------------------------------
  ...
  // If package not found, show error page with debugging info
  if (!pkg) {
    return (
      <div className="min-h-screen flex items-center justify-center p-8 bg-gradient-to-br from-stone-50 to-amber-50">
        <div className="max-w-xl text-center">
          ...
          <div className="bg-gray-100 p-4 rounded-lg mb-6 text-left">
            <p className="text-sm font-mono mb-2">Debugging Information:</p>
            <p className="text-sm font-mono">Package ID: {packageId}</p>
            <p className="text-sm font-mono">
              Available IDs: {Object.keys(packageDetails).join(", ")}
            </p>
          </div>
  ...
  // Prepare short itinerary for Quick Glance section
  const shortItinerary =
    pkg.shortItinerary || pkg.itinerary.map((item) => item.detail);
    ...
    <div className="min-h-screen bg-gradient-to-br from-stone-50 to-amber-50 text-gray-800 font-inter antialiased relative overflow-hidden">
      ...
      <div className="relative h-[400px] overflow-hidden">
        <div
          ...
          style={{ backgroundImage: `url(${pkg.banner || pkg.heroImage})` }}
        ></div>
                ...
                >
                  ...
                  <div className="flex items-center justify-between mb-10">
                    ...
                    <span className="ml-4 text-sm font-semibold px-4 py-1.5 rounded-full bg-yellow-100 text-yellow-800 border border-yellow-300 shadow-sm">
                      {shortItinerary.length} Days
                    </span>
                  </div>

                  {/* Items */}
                  <div className="flex flex-col gap-6">
                    {shortItinerary.map((item, idx) => (
                      <div
                        key={idx}
              ...
              <div className="max-w-6xl mx-auto p-8 px-6">
                ...
                >
                  ...
                  <div className="grid gap-4 md:grid-cols-2">
                    {(pkg.expect || pkg.whatToExpect).map((item, idx) => (
                      <div
                        key={idx}
              ...
              <div className="space-y-3">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">
                    {pkg.start || pkg.location}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600">End Point:</span>
                  <span className="font-medium">{pkg.end || pkg.location}</span>
                </div>
                <div className="flex justify-between">
